...
def proceso(env, t_crea, nombre, ram, mem, ins, ins_x_t):
    ...
    # Se agrega al tiempo total de este proceso (tiempo actual - tiempo de llegada)
    lista.append(env.now - tiempoLlegada)
    tiempoTOTAL += (env.now - tiempoLlegada)  


lista=[] #los tiempos de cada proceso se guarda en esta lista
memoria_ram= 100 #100 unidades de memoria ram
ins_x_t = 3.0 #ejecuta 3 instrucciones por unidad de tiempo
tiempoTOTAL = 0.0 #tiempo total de los procesos
n_procesos = 100 # numero de procesos a ejecutar


env = simpy.Environment()  #se crea el hambiente para la simulacion
...
# Se define la creacion de procesos
...
print "El PROMEDIO de tiempo por cada proceso es ", promedio

#Se trabaja el calculo de desviacion estandar
temp=0
for i in lista:
    temp+=(i-promedio)**2
